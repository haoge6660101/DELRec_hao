...
def createNetwork(featuresLength, activationType='relu', minNeurons=0,
    ...
    for i in range(len(normalizedSet)):
        ...
        for j in range(len(normalizedSet[i])):
            ...
            line["inverse_transform-" + selectedFeaturesNames[j]] = modelOutput[i][j] * std[selectedFeaturesNames[j]] + \
                                                                    means[selectedFeaturesNames[j]]
    ...
    df_results_01.to_csv(
        'out_' + jsonConfig['inputDataSetFile'] + "_dim_" + str(minNeurons) + "_loss_" + str(
            history.history['loss'][-1]) + ".csv", sep=';', float_format='%.15f', encoding='utf-8')
