...
from ..utils import check_suffix, rearrange_string,create_prompt
    ...
    if load_datalines == True:
        ...
        for split in ['train', 'test', 'validation']:
            ...
            for data in dataset[split]:
                ...
                for index, i in enumerate(data['ICL']):
                    if check_suffix(i):
                        data['ICL'][index] = rearrange_string(i)
                for index, i in enumerate(data['user_can']):
                    if check_suffix(i):
                        data['user_can'][index] = rearrange_string(i)
                for index, i in enumerate(data['user_TA']):
                    if check_suffix(i):
                        data['user_TA'][index] = rearrange_string(i)
...
if load_ta == False:
    ...
    df = pd.read_csv(r'transform/TFRS/smovies.csv')#../../transform/TFRS/smovies.csv
    ...
    def TA_data_partition(ff):
        ...
        for user in tqdm(User):
            if len(User[user]) < 5:
                ...
                for _ in range(19):
                    can_item = np.random.choice(list(movies.keys()))
