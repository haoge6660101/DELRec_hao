...
optimizer2 = PagedLion8bit(optimizer_grouped_parameters2, lr=lr,weight_decay=we_de)
...
for epoch in tqdm(range(400)):
    ...
    #torch.cuda.empty_cache()
    for step, (inputs1, inputs2) in zip(enumerate(TA_train_dataloader), enumerate(RPS_train_dataloader)):
        if use_cuda:
        ...
        if actual_step % gradient_accumulation_steps == 0 and glb_step > 0 and glb_step % eval_every_steps == 0:
            ...
            if ht5 > best_ht5 and ht1 > best_ht1 and nd5 > best_nd5 and ht1_val > best_ht1 and ht5_val >best_ht5 and nd5_val > best_nd5:
                ...
                best_ht1 = ht1
                best_nd5 = nd5
                torch.save(prompt_model.state_dict(),f"./last/500_ht1_{ht1:.4f}_ht5_{ht5:.4f}_epoch_{epoch}_lr{lr}_wd{we_de}_t5xl.ckpt")
                print("Save Success!")
            prompt_model.train()

    print(f"/ht1_{ht1:.4f}_ht5_{ht5:.4f}_nd5_{nd5:.4f}")